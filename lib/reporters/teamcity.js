    ...
    _onRunnerEnd: function() {
        ...
        try {
            ...
            this._results.forEach(function(test, index) {
                ...
                if (test.pending) {
                    console.log(tsm.testIgnored({name: testName}).replace('##teamcity', 'tc'));
                    return;
                }

                console.log(tsm.testStarted({name: testName}).replace('##teamcity', 'tc'));

                if (test.state === 'failed') {
                    console.log(tsm.testFailed({
                        name: testName,
                        message: test.err,
                        details: test.err && test.err.stack || test.err
                    }).replace('##teamcity', 'tc'));
                }
                console.log(tsm.testFinished({name: testName, duration: test.duration}).replace('##teamcity', 'tc'));
            }.bind(this));
            ...
            this._results.forEach(function(test, index) {
                ...
                if (test.pending) {
                    console.log(tsm.testIgnored({name: testName}));
                    return;
                }

                console.log(tsm.testStarted({name: testName}));
                ...
                console.log(tsm.testFinished({name: testName, duration: test.duration}));
            }.bind(this));
